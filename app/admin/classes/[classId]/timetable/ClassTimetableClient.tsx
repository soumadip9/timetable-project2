...
interface ClassTimetableClientProps {
  classData: Class;
...
export default function ClassTimetableClient({
  classData,
  timetables: initialTimetables,
  teachers: initialTeachers,
  subjects: initialSubjects,
  rooms: initialRooms,
}: ClassTimetableClientProps) {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [timetablesRes, teachersRes, subjectsRes, roomsRes] = await Promise.all([
        fetch(`/api/timetables?classId=${classData.id}`),
        fetch('/api/teachers'),
        fetch('/api/subjects'),
        fetch('/api/rooms'),
      ]);
      ...
      setTimetables((timetablesData.success ? timetablesData.data : []).map(transformTimetable));
  ...
  const getTimetableForSlot = (day: DayOfWeek, timeSlot: TimeSlot): TimetableWithDetails | null => {
    // Filter timetables for this specific class
    const matchingTimetables = timetables.filter(
      (t) => t.day === day && t.timeSlot === timeSlot && t.classId === classData.id
    );
                    ...
                    return (
                      ...
                      >
                        {entry ? (
                          ...
                          >
                            <div className="font-semibold text-blue-900">{entry.subject.name}</div>
                            <div className="text-blue-700">{entry.teacher.name}</div>
        ...
        <div className="md:hidden space-y-4">
          {DAYS.map((day) => (
            <div key={day} className="rounded-lg border border-gray-200 p-4">
              <h3 className="mb-3 text-lg font-semibold text-gray-900">{day}</h3>
                ...
                {TIME_SLOTS.map((timeSlot) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {entry ? (
                        <div
                          className="flex-1 cursor-pointer rounded bg-blue-100 p-2 text-xs hover:bg-blue-200"
                          onClick={() => handleSlotClick(day, timeSlot)}
                        >
                          <div className="font-semibold text-blue-900">{entry.subject.name}</div>
                          <div className="text-blue-700">{entry.teacher.name}</div>
                        ...
                        <div
                          className="flex-1 cursor-pointer rounded border-2 border-dashed border-gray-300 p-2 text-center text-xs text-gray-600 hover:border-blue-400 hover:text-blue-600"
                          onClick={() => handleSlotClick(day, timeSlot)}
                        >
                          Click to add
                        </div>
      ...
      {isModalOpen && (
        <EntityModal
          ...
          initialData={
            editingTimetable
              ? {
                  day: editingTimetable.day || activeDay,
